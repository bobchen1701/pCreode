...
def down_sample(data, density, noise, target):
    ...
    :param noise:   value for noise threshold, densities below value will
                    be removed during downsampling
    ...
    for i in range(len(density)):
        # remove outliers
        if density[i] < outlier_density:
            cell_prob[i] = 0
        # keep data points within target range
        elif density[i] >= outlier_density and density[i] <= target_density:
            cell_prob[i] = 1
        # set probability for over represented data points
        elif density[i] > target_density:
            cell_prob[i] = float(target_density) / float(density[i])
    ...
    print(
        "Number of data points in downsample = {0}".format(
            len(down_sampled)))
...
def find_closest_ind(point, data):
    ...
    '''
    dist = pairwise_distances(point.reshape(
        1, -1), data, n_jobs=1, metric='l2')
...
def get_graph_distance(from_ind, to_ind, graph):
    ...
    d = np.zeros((F, T))
    for i in range(F):
        d[i, :] = graph.shortest_paths(
            from_ind[i], to_ind, weights="weight")[0]
...
def find_endstates(
    ...
    :param noise:   value for noise threshold, densities below value will
                    be removed during downsampling
    :param target:  value for target density
    :param potential_clusters: value for upper range of number of clusters
                               to search for, default value is 10
    :param cls_thresh: value for closeness threshold use to separate
                       potential endstates from transitional cell
                       types default value is 0.0
    :return endstates_ind: array of indices for most representative cell
                           of identified endstates
    :return down_ind:      ndarray with indices of datapoints selected by
                           downsampling
    :return clust_ids:     ndarray with cluster IDs; highest valued cluster
                           ID is for cells with high closeness, likely
                           transititional cell types
    :return std_cls:       array of containing the closeness values for the
                           downsampled dataset
    '''
    if data is not np.ndarray:
        raise TypeError('`data` must be numpy ndarray')
    if density is not np.ndarray:
        raise TypeError('`density` must be numpy ndarray')


    # get downsampled dataset
    down, down_ind = down_sample(data, density, noise, target)

    # array for orginal density (prior to downsampling) of downsampled data
    # points
    ...
    for z in range(n_down):
        knn_adj[z, np.argsort(Dist[z, :])[1:nn[z]]] = 1
    ...
    dens_norm = np.ravel(norm.fit_transform(
        down_density.reshape(-1, 1).astype(np.float)))
    ...
    d_knn = igraph.Graph.Weighted_Adjacency(
        dist_weighted_adj.tolist(), loops=False, mode=ADJ_UNDIRECTED)
    ...
    # using closeness as threshold (default value = 0.0) get potential
    # endstates
    ...
    for s in range(potential_clusters):
        kmeans_model = KMeans(n_clusters=s + 2, random_state=10).fit(low_cls)
        label = kmeans_model.labels_
        sil_score[s] = metrics.silhouette_score(
            low_cls, labels=label, metric='l2')

    # find most likely number of clusters from scores above and double to
    # allow for rare cell types
    num_clusters = (np.argmax(sil_score) + 2) * 2
    clust_model = KMeans(
        n_clusters=num_clusters,
        random_state=10).fit(low_cls)
    ...
    for i in range(num_clusters):
        endstates_ind[i] = find_closest_ind(endstates[i], data)
    ...
    for i in range(n_down):
        if std_cls[i] <= cls_thresh:
            clust_ids[i] = label[dd]
            dd = dd + 1
        else:
            clust_ids[i] = num_clusters + 1
...
def hierarchical_placement(graph, end_ind):
    '''
    Function for selecting pathways through a graph for selected nodes,
    revealing ancestral relationships
    :param graph:   an igraph weighted graph
    :param end_ind: indices of selected nodes to be placed
    :return hi_pl: igraph weighted graph of hierarchical placed nodes and
                   selected paths connecting them
    :return names: numpy array of hierarchical placed node indices
                   (in terms of down_ind)
    '''
    n_end = len(end_ind)
    ...
    hi_pl.add_vertices([str(i) for i in end_ind])

    # array to hold indices of nodes that are capable of being connected to,
    # initialized with just endstates
    to_ind = end_ind
    # array to hold connected endstates
    conn_ind = np.empty((0, 1), dtype=int)
    # nested array to hold path indices for connected indices
    paths = []
    [paths.append([i][0]) for i in end_ind]
    ...
    while np.any(np.in1d(end_ind, conn_ind, invert=True)):
        ...
        run_dist = pd.DataFrame(
            run_dist,
            index=[str(j) for j in end_ind],
            columns=[str(i) for i in to_ind])

        # set dist to used coords to unrealistic min dist
        # simply removing rows caused an accounting headache, we be fixed in
        # future update
        ...
        run_path = graph.get_shortest_paths(
            end_ind[run_coords[0]], to_ind[run_coords[1]], weights="weight")[0]
        ...
        hi_pl.add_vertices([str(i) for i in unq_path])
        # connect edges in new graph to recreate path
        for es_itr in range(len(run_path) - 1):
            wgt = graph.shortest_paths(
                run_path[es_itr], run_path[es_itr + 1], weights="weight")[0][0]
            hi_pl.add_edge(str(run_path[es_itr]), str(
                run_path[es_itr + 1]), weight=wgt)

        for coord_itr in run_coords:
            if coord_itr < n_end:
                conn_ind = np.append(conn_ind, end_ind[coord_itr])
                paths[coord_itr] = np.unique(
                    np.append(paths[coord_itr], run_path))
    ...
    while num_comp > 1:
        ...
        for j in range(1, num_comp):
            rest = np.append(rest, names[comp[j]])
        ...
        comp_path = graph.get_shortest_paths(
            names[comp[0]][comp_coords[0]], rest[comp_coords[1]], weights="weight")[0]
        # add path vertices to hi_pl, if not already in present
        for k in comp_path:
            if np.in1d(k, names):
                continue
            else:
                hi_pl.add_vertices(str(k))
        # connect edges in new graph to recreate path
        for xx in range(len(comp_path) - 1):
            wgt = graph.shortest_paths(
                comp_path[xx], comp_path[xx + 1], weights="weight")[0][0]
            hi_pl.add_edge(str(comp_path[xx]), str(
                comp_path[xx + 1]), weight=wgt)
        comp = hi_pl.components(mode=WEAK)
        num_comp = len(comp)
        names = np.array(hi_pl.vs['name'])
        names = names.astype(np.int)
...
def consensus_alignment(down, hi_pl_ind, data, density, noise):
    ...
    :param hi_pl: igraph weighted graph of hierarchical placed nodes
                  and selected paths connecting them
    :param noise: value for noise threshold, densities below value
                  will be removed during downsampling
    :param data:  numpy ndarray orignal data set
    :param density: numpy array of calculated densities for each
                    datapoint in orginal dataset
    :param noise: value for noise threshold, densities below value
                  will not be considered during alignment
    :return: ndarry of indices of new nodes in terms of location
             within downsampled dataset
    '''
    ...
    for s in range(len(chunks)):
        ...
        gd_dist = pairwise_distances(
            no_noise[chunks[s]], gp, n_jobs=1, metric='l2')

        # array to hold indices for the closest down sampled point
        node_ind = np.zeros(len(chunks[s]), dtype=int)

        for i in range(len(chunks[s])):
            # find closest aligned indice for each point in chunk set
            node_ind[i] = hi_pl_ind[np.argmin(gd_dist[i])]
        for j in range(len(hi_pl_ind)):
            # get the center of binned data points
            if np.in1d(hi_pl_ind[j], node_ind):
                new_nodes[j] = np.median(
                    no_noise[chunks[s][node_ind == hi_pl_ind[j]]], axis=0)
                new_ind[j] = find_closest_ind(new_nodes[j], down)
            else:
                new_nodes[j] = gp[j]
...
def graph_differences(ind_1, ind_2, dist_1, dist_2, dist, g_1, g_2):
    ...
    for i in range(num_y - 1):
        ...
        min_ind_i = np.argmin(dist[i, :])
        # get euclidean distance between closest nodes(transformation distance)
        min_dist_i = dist[i, min_ind_i]
        for j in range(i + 1, num_y):
            # find the other closest node in 2nd graph
            min_ind_j = np.argmin(dist[j, :])
            min_dist_j = dist[j, min_ind_j]
            # count number of branch points two nodes in graph one
            shortest_paths = g_1.get_all_shortest_paths(i, j)[0]
            degree = g_1.degree()
            deg_list_1 = np.transpose(degree)[shortest_paths]
            g1_branches = sum(deg_list_1[deg_list_1 > 2]) - \
                2 * len(deg_list_1[deg_list_1 > 2])
            # count number of branch points two nodes in graph two
            shortest_paths = g_2.get_all_shortest_paths(min_ind_i, min_ind_j)[0]
            degree = g_2.degree()
            deg_list_2 = np.transpose(degree)[shortest_paths]
            g2_branches = sum(deg_list_2[deg_list_2 > 2]) - \
                2 * len(deg_list_2[deg_list_2 > 2])
            # get difference in branch points between the two nodes being
            # compared
            branch_diff = branch_diff + abs(g1_branches - g2_branches)
            # get difference in graph distances plus transformation distance
            # between two nodes in question
            dis_y = dis_y + abs(dist_2[min_ind_i, min_ind_j] -
                                dist_1[i, j] + min_dist_i + min_dist_j)
            count = count + 1
...
def pCreode_Scoring(data, file_path, num_graphs):
    ...
    :param num_graphs:  number of graphs to be scored
    :return branch_diss: value of pairwise difference in branch counts
    :return dist_diss:   value of pairwise difference in graph distance
    '''
    # if data is not np.ndarray:
    ...
    # loop through to compare all graphs, will result in lower left triangle
    # matrix (br&diff)
    for z in range(num_graphs - 1):
        print("scoring graph {0}".format(z + 1))
        for k in range(z + 1, num_graphs):
            # read in arrays for indices for two graphs, in terms of original
            # dataset
            ind_1 = np.genfromtxt(
                file_path + 'ind_{}.csv'.format(z),
                delimiter=',').astype(int)
            ind_2 = np.genfromtxt(
                file_path + 'ind_{}.csv'.format(k),
                delimiter=',').astype(int)
            ...
            adj_2 = pd.read_csv(
                ...
                sep=" ",
                header=None).values
            # get euclidean distance between nodes in each graph to create
            # weights in graph
            dist_1a = pairwise_distances(
                data[ind_1, :], data[ind_1, :], n_jobs=1, metric='l2')
            dist_2a = pairwise_distances(
                data[ind_2, :], data[ind_2, :], n_jobs=1, metric='l2')
            # create weighted adjacency matrix for graphs,
            # original graphs are not used because edges were weighted by
            # density
            wad_1 = np.multiply(dist_1a, adj_1)
            wad_2 = np.multiply(dist_2a, adj_2)
            # create igraph weighted graph objects
            g1 = igraph.Graph.Weighted_Adjacency(
                wad_1.tolist(), mode=ADJ_UNDIRECTED)
            g2 = igraph.Graph.Weighted_Adjacency(
                wad_2.tolist(), mode=ADJ_UNDIRECTED)
            # get graph distance between all nodes in each graph
            ind_1_len = len(ind_1)
            ind_2_len = len(ind_2)
            dist_1 = get_graph_distance(range(ind_1_len), range(ind_1_len), g1)
            dist_2 = get_graph_distance(range(ind_2_len), range(ind_2_len), g2)
            # get euclidean distance between node in graph 1 and graph 2
            # (transformation distances)
            dist = pairwise_distances(
                data[ind_1, :], data[ind_2, :], n_jobs=1, metric='l2')
            ...
            br_x, diff_x = graph_differences(
                ind_1, ind_2, dist_1, dist_2, dist, g1, g2)
            br_y, diff_y = graph_differences(
                ind_2, ind_1, dist_2, dist_1, dist.T, g2, g1)
            # the max is taken to make the score symmetric
            br[z, k] = max(br_x, br_y)
            diff[z, k] = max(diff_x, diff_y)
...
def pCreode(
        ...
        start_id=0):
    '''
    Function for running full pCreode algorithm, with the addition of principle component
    extremes found to be under the closeness threshold added as endstates
    :param data:    numpy ndarray of data set
    :param density: numpy array of calculated densities for each datapoint
    :param noise:   value for noise threshold, densities below value will be removed
                    during downsampling
    :param target:  value for target density
    ...
    :param potential_clusters: value for upper range of number of clusters to search
                               for, default value is 10
    :param cls_thresh: value for closeness threshold use to separate potential endstates
                       from transitional cell types default value is 0.0
    :param start_id: integer at which to start labeling output graphs, allows for
    ...
    for run_itr in range(num_runs):
        ...
        # set upper and lower thresholds for number of neighbors to connect in density
        # based nearest neighbor graph (d-kNN) (current fixed values are 2 and
        # 10)
        upper_nn = 10
        lower_nn = 2
        ...
        print("finding endstates")
        # get closeness of graph and standardize to aid in endstate
        # identification
        cls = np.array(d_knn.closeness(weights="weight"))
        scaler = preprocessing.StandardScaler()
        std_cls = scaler.fit_transform(cls.reshape(-1, 1)).ravel()
        ...
        # Endstate data points were picked from full data set, so need
        # to be appended to down and down_ind
        # Create array to hold where end_states are located within the
        # downsampled dataset
        ...
        for es in range(num_clusters):
            ...
            # if data point is already in down
                ...
                cen_ind[es] = np.argwhere(
                    endstates_ind[es] == down_ind).ravel()[0]
                continue

        # re-initialize using density and distance weighted edges
        dens_knn = igraph.Graph.Weighted_Adjacency(
            dens_weighted_adj.tolist(), loops=False, mode=ADJ_UNDIRECTED)

        # add endstate data points to the already constructed dens_knn graph,
        # connecting to 2 closest neighbors
        # future update will so that number of edges is based on density of
        # data point
        knn_num = 2
        # add nodes to graph that will represent endstates
        dens_knn.add_vertices(num_clusters)
        # get distance matrix to be used for finding closeset neighbors in
        # graph
        end_dist = np.array(pairwise_distances(
            endstates, down[:-num_clusters], n_jobs=1))
        for k in range(num_clusters):
            edg_wts = np.sort(end_dist[k, :])[1:knn_num + 1]
            edg_ids = np.argsort(end_dist[k, :])[1:knn_num + 1]
            for j in range(knn_num):
                # no need to connect if connection is already present
                if edg_wts[j] < 2.0e-06:
                    continue
                # if not present add edge with distance/density weight
                else:
                    dens_knn.add_edge(
                        cen_ind[k], edg_ids[j], weight=edg_wts[j] * (1 - dens_norm[edg_ids[j]]))

        print("hierarchical placing")
        # perform hierarchical placement of endstates (find shortest paths
        # connecting them within d_knn)
        ...
        aligned_ind = consensus_alignment(
            down, hi_pl_ind.copy(), data, density, noise)
        # add orginal endstates back into aligned list of indices
        al_es_ind = np.append(cen_ind, np.unique(
            aligned_ind[~np.in1d(aligned_ind, cen_ind)]))
        ...
        creode_graph, creode_ind = hierarchical_placement(
            al_hi_pl, range(len(cen_ind)))
        creode_graph.simplify(combine_edges="mean")
        print("saving files for run_num {0}".format(run_itr + 1))
        np.savetxt(file_path + "ind_{0}.csv".format(start_id),
                   down_ind[al_hi_pl_ind[creode_ind]], delimiter=',')
        creode_graph.save(
            file_path +
            "adj_{0}.txt".format(start_id),
            format="adjacency")
...
def pCreode_pca_extremes(
        ...
        cls_thresh=0.0):
    '''
    Function for running full pCreode algorithm, with the addition of principle
    component extremes found to be under the closeness threshold added as endstates
    :param data:    numpy ndarray of data set
    :param density: numpy array of calculated densities for each datapoint
    :param noise:   value for noise threshold, densities below value will be
    ...
    :param potential_clusters: value for upper range of number of clusters to search
                               for, default value is 10
    :param cls_thresh: value for closeness threshold use to separate potential
                       endstates from transitional cell types default value is 0.0
    :return: will save creode files in given directory
    '''
    if data is not np.ndarray:
        raise TypeError('`data` must be numpy ndarray')
    ...
    print(
        "Performing {0} independent runs, may take some time".format(num_runs))
    ...
    for run_itr in range(num_runs):
        ...
        # Endstate data points were picked from full data set,
        # so need to be appended to down and down_ind
        # Create array to hold where end_states are located within the
        # downsampled dataset
        cen_ind = np.zeros(num_clusters, dtype=int)
        ind = n_down
        ...
        creode_graph.simplify(combine_edges="mean")
        print("saving files for run_num {0}".format(run_itr + 1))
        np.savetxt(file_path + "ind_{0}.csv".format(run_itr),
                   down_ind[al_hi_pl_ind[creode_ind]], delimiter=',')
        creode_graph.save(
            file_path +
            "adj_{0}.txt".format(run_itr),
            format="adjacency")
...
def return_weighted_adj(data, file_path, graph_id):
    ...
    ind_1 = np.genfromtxt(
        file_path +
        'ind_{}.csv'.format(graph_id),
        delimiter=',').astype(int)
    # read adjacency matrix for graphs
    adj_1 = pd.read_csv(
        file_path +
        'adj_{}.txt'.format(graph_id),
        sep=" ",
        header=None).values
    # get euclidean distance between nodes in each graph to create weights in
    # graph
    dist_1a = pairwise_distances(
        data[ind_1, :], data[ind_1, :], n_jobs=1, metric='l2')
    # create weighted adjacency matrix for graphs,
...
def pCreode_rare_cells(
        ...
        mute=False):
    '''
    Function for running full pCreode algorithm, but allows user to add rare
    cell clusters using the indices of the cells.
    :param density: numpy array of calculated densities for each datapoint
    :param noise:   value for noise threshold, densities below value will be
    removed during downsampling
    :param target:  value for target density
    :param file_path: path to directory where output files will be stored
    :param rare_clusters: nested array consisting of indices for clusters to
    be added, format of [[clust1 indices],[clust2 indices],...]
    :param num_runs:  number of independent runs to perform, default is 100
    :param potential_clusters: value for upper range of number of clusters to
    search for, default value is 10
    :param cls_thresh: value for closeness threshold use to separate potential
    endstates from transitional cell types default value is 0.0
    :param start_id: integer at which to start labeling output graphs, allows
    for addition of graphs to previously ran lot
    :return: will save creode files in given directory
    '''
    if data is not np.ndarray:
        raise TypeError('`data` must be numpy ndarray')
    ...
    for run_itr in range(num_runs):
        ...
        for clust_itr in range(len(rare_clusters)):
            rare_clusters_center = np.median(
                data[rare_clusters[clust_itr]], axis=0)
        ...
        # add endstate data points to the already constructed dens_knn
        # graph, connecting to 2 closest neighbors
        # future update will so that number of edges is based on density of
        # data point
        knn_num = 2
        # add nodes to graph that will represent endstates
...
def pCreode_supervised(
        ...
        start_id=0):
    '''
    Function for running full pCreode algorithm, without endstate identification.
    Relies on user supplied endstates.
    :param data:    numpy ndarray of data set
    :param density: numpy array of calculated densities for each datapoint
    :param noise:   value for noise threshold, densities below value will be
    removed during downsampling
    :param target:  value for target density
    :param file_path: path to directory where output files will be stored
    :param man_clust: nest array consisting of indices for clusters to be added,
    format of [[clust1 indices],[clust2 indices],[clust3 indices],...]
    :param num_runs:  number of independent runs to perform, default is 100
    :param start_id: integer at which to start labeling output graphs, allows
    for addition of graphs to previously ran lot
    :return: will save creode files in given directory
...
def pCreode_sparse(
    ...
    for, default value is 10
    :param cls_thresh: value for closeness threshold use to separate potential
    endstates from transitional cell types default value is 1.0
    :param start_id: integer at which to start labeling output graphs, allows for
    addition of graphs to previously ran lot
    :return: will save creode files in given directory
    '''
    ...
    for run_itr in range(num_runs):
        ...
        # Endstate data points were picked from full data set, so
        # need to be appended to down and down_ind
        # Create array to hold where end_states are located within the
        # downsampled dataset
        cen_ind = np.zeros(num_clusters, dtype=int)
        ind = n_down
...
def pCreode_extended_trajectories(
        data,
        density,
        noise,
        target,
    ...
    :param start_id: integer at which to start labeling output graphs, allows for
    addition of graphs to previously ran lot
    :param clust_scaler: scaling factor for number of clusters, default is 2 to help
    ...
    for run_itr in range(num_runs):
        ...
        # find most likely number of clusters from scores above and scale by
        # clust_fact
        ...
        for i in range(num_clusters):
            # get the coorindates for each cell in cluster i
            clust_inds = np.argwhere(i == label)
            ...
            # pull and sort closeness values, pulling 10 highest for finding
            # center
            high_cls_args = np.argsort(std_cls[cls_mask][clust_inds].ravel())[
                :min(len(clust_inds), 10)]
            # median for highest ten data points are the new centers
            endstate = np.mean(clust_data[high_cls_args], axis=0)
            endstates_ind = np.append(
                endstates_ind, find_closest_ind(
                    endstate, data))
