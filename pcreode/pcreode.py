...
class PCA(object):
    ...
    def get_pca(self):  # gets a PCA of the data the object was initialized with
        ...
        # output a percent of the variance explained within the object defined
        # here
    ...
    def pca_plot_explained_var(self, figsize=(6, 6), xlim=(0, 25)):
        ...
        ax.set_xlim(xlim)
        ax.plot(range(len(self.pca_explained_var)),
                self.pca_explained_var, '-o')
        return
    ...
    def pca_set_components(self, n_components):
        ...
        if self.pca_explained_var is None:
            raise RuntimeError(
                'Please run get_pca() before selecting components')
...
class Density(object):
    ...
    def __init__(self, preprocessed_data, metric='euclidean'):
        ...
        if (~np.in1d(metric, ['cityblock', 'cosine',
                              'euclidean', 'l1', 'l2', 'manhattan'])):
            raise TypeError(
                'metric must be one of the following cityblock, cosine, euclidean, l1, l2, or manhattan')
    ...
    def nearest_neighbor_hist(
        ...
        dists = _pairwise_distances(
            self._data[r_inds, :], self._data, metric=self._metric)
    ...
    def get_density(self, radius, chunk_size=5000, mute=False):
        ...
        # Due to memory restrictions density assignments have to be preformed
        # in chunks
        ...
        for chunk in all_chunks:
            ...
            chunk_dist = _pairwise_distances(
                self._data[chunk, :], self._data, n_jobs=1, metric=self._metric)
            print(
                "calculating densities for datapoints: {0} -> {1}".format(chunk[0], chunk[-1]))

            for chunk_ind, ind in enumerate(chunk):
                neighbors[ind] = np.setdiff1d(
                    np.ravel(
                        np.argwhere(
                            chunk_dist[chunk_ind] <= radius).ravel()),
                    ind)
...
class Analysis(object):
    ...
    def __init__(
        ...
        # node_data_indices are the node indices from the data set used to run
        # pCreode
        self.node_data_indices = np.genfromtxt(
            file_path +
            'ind_{}.csv'.format(
                self._graph_id),
        ...
        self.dist = pairwise_distances(
            self._data[self.node_data_indices, :], self._data[self.node_data_indices, :], n_jobs=1, metric='l2')
        # calculate weighted adjacency matric
        w_adj = np.multiply(adj.values, self.dist)
        # create graph to plots
        self.graph = _igraph.Graph.Weighted_Adjacency(
            w_adj.tolist(), mode=ADJ_UNDIRECTED)
    ...
    def plot_save_graph(
        ...
        dens = norm_dens.fit_transform(self._density.astype(
            float).reshape(-1, 1))[self.node_data_indices]
        # normalize overlay to use for node overlays
        norm_ana = preprocessing.MinMaxScaler(feature_range=(0, upper_range))
        norm_ana.fit(overlay[self._density > self._noise].values.astype(
            np.float).reshape(-1, 1))
        old_ana = norm_ana.transform(
            overlay[self._density > self._noise].values.astype(np.float).reshape(-1, 1))
        # bin the data points to each node so that an average of closest
        # surrounding nodes is used for overlay
        bin_dist = pairwise_distances(
            self.good_cells, self._data[self.node_data_indices])
        ...
        self.graph.vs["color"] = [cl_vals_1[kk]
                                  for kk in range(self.num_nodes)]
        random.seed(seed)
        ...
        plot(
            ...
            vertex_size=dens,
            edge_width=2,
            vertex_label_size=node_label_size)
    ...
    def plot_analyte_dynamics(self, overlay, root_id):
        ...
        """
        # get all end-state nodes, including root if also an end-state node
        # (degree==1)
        all_end_id = self.node_graph_indices[np.transpose(
            self.graph.degree()) == 1]
        ...
        for ii in range(self.num_nodes):
            ...
            new_std[ii] = np.std(old_ana[bin_assignments == ii]) / \
                float(np.sqrt(len(old_ana[bin_assignments == ii])))
        # get branch points
        branch_id = self.node_graph_indices[np.transpose(
            self.graph.degree()) >= 3]
        ...
        for ii in range(num_traj):
            ...
            _plt.bar(range(len(new_ana[traj[ii]])),
                     new_ana[traj[ii]],
                     width=1.0,
                     color='green',
                     yerr=new_std[traj[ii]])
            _plt.ylim(0, max(new_ana) + max(new_std))
            _plt.xlim(0, xlim)
            # plot where trajectory ends
            _plt.axvline(x=len(new_ana[traj[ii]]),
                         color='black',
                         linewidth=2.5,
                         linestyle='--')
    ...
    def get_complete_analyte_gene_trajectories(
            self, overlay_data, root_id, file_out):
        """
        ...
        # lazy work around for when ravel is not needed with non-branching
        # trajectories
        if(len(end_ids) == 1):
            traj = [traj]
        ...
        np.savetxt(
            self._file_path +
            "{}_clust_ids.csv".format(file_out),
            np.vstack(
                (self.good_cells_inds,
                 bin_assignments)),
            delimiter=',')
        ...
        for cc in range(num_traj):
            ...
            traj_ana = pd.DataFrame(
                new_ana[traj[cc]].T, index=overlay_data.columns, columns=traj[cc])
            traj_ana.to_csv(
                self._file_path +
                '{0}_traj{1}_analytes.csv'.format(
                    file_out,
                    cc +
                    1))
    ...
    def plot_save_qual_graph(self, seed, overlay, file_out):
        ...
        if not (overlay.dtype.char == 'U'):
            raise TypeError(
                'All elements in overlay variable must be in a string dtype')
        ...
        for ii in range(self.num_nodes):
            ...
            uniqs = np.unique(
                overlay[np.where(bin_assignments == ii)], return_counts=True)[1]
